...
//      simpleAuthorInfo.addStringPermissions(permissionList);  
        ...
        if(null != currentUsername && "admin".equals(currentUsername)){  
            ...
            System.out.println("已为用户[admin]赋予了[user,admin]角色和[admin:manage]权限");  
        ...
        if(null != currentUsername && "user".equals(currentUsername)){
            ...
            simpleAuthorInfo.addRole("user");  
            //添加权限  
            simpleAuthorInfo.addStringPermission("user:view");
            return simpleAuthorInfo;  
        }
...
//      }  
        ...
        if("admin".equals(token.getUsername())){  
            AuthenticationInfo authcInfo = new SimpleAuthenticationInfo("admin", "admin123", this.getName());  
            this.setSession("currentUser", "admin");  
            return authcInfo;  
        }

        if("user".equals(token.getUsername())){  
            AuthenticationInfo authcInfo = new SimpleAuthenticationInfo("user", "user123", this.getName());  
            this.setSession("currentUser", "user");  
            return authcInfo;  
        }
